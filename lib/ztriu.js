...
* // B => <Complex128Array>[ 0.0, 0.0, 3.0, 4.0, 0.0, 0.0, 0.0, 0.0 ]
*/
function ztriu( order, M, N, k, A, LDA, B, LDB ) {
	var isrm;
	var sa1;
	var sa2;
	var sb1;
	...
	if ( l === null ) {
		throw new TypeError( format( '38TFx', order ) );
	}
	isrm = isRowMajor( l );
	if ( isrm ) {
		s = N;
	...
	if ( LDA < max( 1, s ) ) {
		throw new RangeError( format( '38TJL', s, LDA ) );
	}
	if ( LDB < max( 1, s ) ) {
		throw new RangeError( format( '38TGM', s, LDB ) );
	}
	if ( isrm ) {
		sa1 = LDA;
		sa2 = 1;
